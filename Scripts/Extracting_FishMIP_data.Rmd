...
join_effort_data <- function(file_name, mask, prefix_name){
  ...
  if(!file.exists(this_destination_path)){
    ...
    these_aggregated_data <-  these_data %>%
      ...
      mutate(Year = Year) %>%
      #Save file
      write_csv(file = this_destination_path)
  }
}
```
...
for(i in seq_along(effort_list_split)){
  ...
  mclapply(X = file_chunk, FUN = join_effort_data, mask_df, "05deg", mc.cores = 40)
}
```
...
mclapply(X = newly_written_files, FUN = read_csv, mc.cores = 40) %>% 
  ...
  write_csv(paste0(aggregated_files_dir, "05deg/05deg_all_effort_aggregated.csv", sep = "/"))
```
